...
for (ik in seq_along(phytos)) {
  ...
  pdat=spydat
  pdat$col=1
  pdat$col=ifelse(pdat$year>1994, 4, 3)
  ...
  plot(metric~TP, data=pdat, type="b",xaxt="n",pch=16,
       col=pdat$col,
       #col="grey",
       ann=FALSE,
       #xlim=xyear,
  ...
  axis(side=1, las=1)
  mtext(mmetric,side=3)
  text(x=pdat$TP, y=pdat$metric, labels=pdat$year, cex= 0.7, pos=3)
 ...
 newx <- seq(min(pdat$TP), max(pdat$TP), length.out=100)
 preds <- predict(tp1, newdata = data.frame(TP=newx), 
                  interval = 'confidence')
