...
best=tp10
mpred=as.data.frame(predict(tp10, spydat, interval="confidence"))
...
#     bfac.ri= fac.ri
  ...
  plot(metric~year, data=pdat, type="b",xaxt="n",pch=16,col="grey",
       ann=FALSE, xlim=xyear, ylim=c(0,2.25),las=2,yaxp=c(0,2.,4))
...
#  if (i==3) hgt=4
  ...
  if (tm==1) {
    ...
    text(pdat$year[ci[2]]-3,2.2, pdat$year[ci[2]])
