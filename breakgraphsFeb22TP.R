...
#     bfac.ri= fac.ri
  ...
  fm.ri <- lm(metric ~ time+year, data=pdat, weights=weights)
  fm.riN <- lm(metric ~ year, data=pdat, weights=weights)
  AIC(fm.ri, fm.riN)
  summary(fm.ri)
  } else {
    fm.ri <- lm(metric ~ year, data=pdat)
  }
  ...
  plot(metric~year, data=pdat, type="b",xaxt="n",pch=16,col="grey",
       ann=FALSE, xlim=xyear, ylim=c(0,0.105),las=2,yaxp=c(0,0.1,5))
...
#  if (i==3) hgt=4
  ...
  if (tm==1) {
    ...
    text(pdat$year[ci[2]]-3,0.1, pdat$year[ci[2]])
  ...
  segments(xn[s], yn[s], xx[s], yx[s], col="blue", lwd=2, lend="square")
  abline(v=pdat$year[ci[,2]], lty=2, col="blue")
  text(pdat$year[ci[,2]]-3,0.105, pdat$year[ci[,2]])
  }
