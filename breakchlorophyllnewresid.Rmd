...
### Residual breakpoint analysis for chlorophyll a
...
if ( i==3)axis(side=1, las=1)
 ...
 mtext(mlab, side = 2, line = 0, outer = TRUE, at = NA,
  ...
  treg=as.data.frame(summary(tp3)$coefficients)
...
In this case the best fit linear models included site-specific slopes. There was no difference in AIC (`r c(cop[1,2], cop[3,2])`) or R2 (`r c(summary(tp1)$adj.r.squared,summary(tp2)$adj.r.squared)`) between a site specific linear models of chlorophyll a vs total phosophorus with intercept given by site, and a model with slope and intercept specific to each site. both models performed better than a model with a global slope and individual intercepts  (AIC=`r cop[2,2]`. R^2=`r  summary(tp2)$adj.r.squared`). We therefore used the site-specific slope model (fig & table) to calculate residuals.
...
There were no differences in models with site-specific slopes, vs a global slope (AIC=`r copref[3,2]`, $R^2$=`r summary(tp6)$adj.r.squared` vs AIC=`r copref[2,2]`, $R^2$=`r summary(tp5)$adj.r.squared` for a model with a global slope), and site specific intercepts as well as slope (AIC=`r copref[1,2]`, $R^2$=`r summary(tp4)$adj.r.squared`). 
...
#  if (i==3) hgt=4
  ...
  mtext("Residual chlorophyll a", side = 2, line = 0, outer = TRUE, at = NA,
        adj = NA, padj = NA, cex = 1.4, col = NA, font = NA)
...
regco=kable((regout), digits=2,
            #format="html", table.attr = "style='width:60%;'",
   caption=paste("residual chla","Site", stvec[i]))
tsave=footnote(kable_classic(regco),
