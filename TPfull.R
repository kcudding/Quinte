...
#####
par(mfcol=c(3,3), mar=c(0.5,5,1,1), oma=c(4,1,0,0),
...
mtext(mlab, side = 2, line = -2, outer = TRUE, at = NA,
...
for (i in 1:3){
  ...
  plot(datsres[,yname]~year, xaxt="n",pch=16,col="grey", 
       data=datsres,
       ann=FALSE, xlim=xyear,ylim=c(-15,15),las=2)
 ...
 # points(mgres~year, data=datsres, col="red")
  ...
  if (i==2) mtext("Partial effect", side = 2, line = 3, outer = FALSE, at = NA,
                adj = NA, padj = NA, cex = 1.4, col = NA, font = NA)
  if ( i==3)axis(side=1, las=1)
...
for (j in seq_along(dseq)) {
  ...
  plot(derivative~data, data=dat,xaxt="n",ann=FALSE,
       ylim=yaxl, typ="l", lwd=2, col="dark red")
  ...
  abline(v=1994, col="grey", lwd=2)
  if (i==2) text(1997, 1.45, "1994", cex=1.2)
  if (i==3)axis(side=1, las=1)
  if (i==4) mtext("Derivative of smooth", side = 2, line = 3, outer = FALSE, at = NA,
                  adj = NA, padj = NA, cex = 1.4, col = NA, font = NA)
